...
class AgentRegisterRequest(BaseModel):
    ...
    job_title: str = Field("", description="业务岗位: 产品经理/UI设计师/前端开发/后端开发/数据库开发/运维工程师")
    specialties: list = Field(default_factory=list, description="擅长的任务类别: product/ui_design/frontend/backend/database/devops")
...
async def register_agent(
    ...
    try:
        agent = agent_service.register_agent(
            db, req.name, req.role, req.description,
            job_title=req.job_title, specialties=req.specialties,
        )
...
@router.get("/match", response_model=List[AgentResponse], summary="按任务类别匹配 Agent")
async def match_agents(
    category: str,
    agent: Agent = Depends(get_current_agent),
    db: Session = Depends(get_db),
):
    """根据任务类别查找匹配的 Agent（specialties 包含该类别），按积分降序"""
    try:
        matched = agent_service.find_agents_by_category(db, category)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return matched

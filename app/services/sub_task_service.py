...
def create_sub_task(
    ...
    """创建子任务，支持按 category 自动路由和校验"""
    from app.services.agent_service import find_agents_by_category, TASK_CATEGORIES
    ...
    # 自动路由：当指定 category + auto_assign 且未指定 agent 时，自动匹配
    if category and auto_assign and not assigned_agent:
        matched = find_agents_by_category(db, category)
        if not matched:
            raise ValueError(f"没有找到擅长 '{category}' 的可用 Agent，请手动指派")
        assigned_agent = matched[0].id  # 选积分最高的
    ...
    if assigned_agent:
        ...
        # 专业匹配校验：当指定了 category 且 agent 有 specialties 时校验
        if category and category != "general" and agent.specialties:
            if category not in agent.specialties:
                raise ValueError(
                    f"Agent {agent.name}({agent.job_title or agent.role}) 的专业方向是 {agent.specialties}，"
                    f"与任务类别 '{category}' 不匹配。"
                    f"请使用 agents match --category {category} 查找合适的 Agent"
                )

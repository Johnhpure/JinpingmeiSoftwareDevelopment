...
def cmd_register(args):
    ...
    r = requests.post(
        ...
        json={
            ...
            "specialties": args.specialties or [],
        },
    )
    if r.status_code >= 400:
...
def cmd_sub_task_create(args):
    ...
    agent_info = f" → Agent:{data.get('assigned_agent', '-')}" if data.get('assigned_agent') else ""
    print(f"✅ 子任务已创建: {data['id']}{agent_info}")
    _print_json(data)
...
def cmd_agent_list(args):
    ...
    for a in data:
        job = f" [{a.get('job_title', '')}]" if a.get('job_title') else ""
        specs = f" 擅长:{','.join(a.get('specialties', []))}" if a.get('specialties') else ""
        desc = f" — {a['description']}" if a.get('description') else ""
        print(f"  [{a['status']}] {a['name']} ({a['role']}){job} 积分:{a['total_score']}{specs}{desc}")


def cmd_agent_match(args):
    """按任务类别匹配 Agent"""
    data = _request("get", "/agents/match", args.key, params={"category": args.category})
    if not data:
        print(f"没有找到擅长 '{args.category}' 的 Agent")
        return
    print(f"  匹配 '{args.category}' 的 Agent：")
    for a in data:
        job = f" [{a.get('job_title', '')}]" if a.get('job_title') else ""
        print(f"  {a['name']} (ID:{a['id']}){job} 积分:{a['total_score']}")
...
def cmd_escalation_create(args):
    """创建任务上报"""
    body = {
        "task_id": args.task_id,
        "title": args.title,
        "description": args.desc or "",
        "suggested_category": args.category or "",
    }
    if args.sub_task_id:
        body["source_sub_task_id"] = args.sub_task_id
    data = _request("post", "/escalations", args.key, json=body)
    print(f"✅ 已上报: {data['title']} (ID:{data['id']})")


def cmd_escalation_list(args):
    """查看上报列表"""
    params = {}
    if args.task_id:
        params["task_id"] = args.task_id
    if args.status:
        params["status"] = args.status
    data = _request("get", "/escalations", args.key, params=params)
    if not data:
        print("暂无上报记录")
        return
    for e in data:
        cat = f" [{e.get('suggested_category', '')}]" if e.get('suggested_category') else ""
        print(f"  [{e['status']}] {e['title']}{cat} (ID:{e['id']})")


def cmd_escalation_get(args):
    """查看上报详情"""
    data = _request("get", f"/escalations/{args.id}", args.key)
    _print_json(data)


def cmd_escalation_accept(args):
    """接受上报"""
    body = {}
    if args.sub_task_id:
        body["result_sub_task_id"] = args.sub_task_id
    data = _request("put", f"/escalations/{args.id}/accept", args.key, json=body)
    print(f"✅ 已接受上报: {data['title']}")


def cmd_escalation_reject(args):
    """拒绝上报"""
    body = {"reason": args.reason or ""}
    data = _request("put", f"/escalations/{args.id}/reject", args.key, json=body)
    print(f"❌ 已拒绝上报: {data['title']}")
...
def main():
    ...
    p.add_argument("--job-title", help="业务岗位（如 产品经理/前端开发）")
    p.add_argument("--specialties", nargs="+", help="擅长的任务类别（如 product ui_design frontend）")
    ...
    p.add_argument("--category", help="任务类别: product/ui_design/frontend/backend/database/devops/general")
    p.add_argument("--auto-assign", action="store_true", help="根据 category 自动匹配 Agent")
    ...
    # --- agents ---
    agents_p = subparsers.add_parser("agents", help="查看 Agent 列表")
    agents_sub = agents_p.add_subparsers(dest="agents_cmd")

    # agents 不带子命令时列出全部
    agents_p.add_argument("--role", help="按角色过滤")
    agents_p.set_defaults(func=cmd_agent_list)

    p = agents_sub.add_parser("match", help="按任务类别匹配 Agent")
    p.add_argument("--category", required=True, help="任务类别: product/ui_design/frontend/backend/database/devops")
    p.set_defaults(func=cmd_agent_match)

    # --- escalation ---
    esc_p = subparsers.add_parser("escalate", help="上报新需求给 Planner")
    esc_p.add_argument("task_id", help="所属任务 ID")
    esc_p.add_argument("title", help="上报标题")
    esc_p.add_argument("--desc", help="需求描述")
    esc_p.add_argument("--category", help="建议的任务类别")
    esc_p.add_argument("--sub-task-id", help="来源子任务 ID")
    esc_p.set_defaults(func=cmd_escalation_create)

    esc_mgr_p = subparsers.add_parser("escalation", help="上报管理")
    esc_mgr_sub = esc_mgr_p.add_subparsers(dest="esc_cmd")

    p = esc_mgr_sub.add_parser("list", help="查看上报列表")
    p.add_argument("--task-id", help="按任务过滤")
    p.add_argument("--status", help="按状态过滤: pending/accepted/rejected")
    p.set_defaults(func=cmd_escalation_list)

    p = esc_mgr_sub.add_parser("get", help="查看上报详情")
    p.add_argument("id", help="上报 ID")
    p.set_defaults(func=cmd_escalation_get)

    p = esc_mgr_sub.add_parser("accept", help="接受上报")
    p.add_argument("id", help="上报 ID")
    p.add_argument("--sub-task-id", help="关联已创建的子任务 ID")
    p.set_defaults(func=cmd_escalation_accept)

    p = esc_mgr_sub.add_parser("reject", help="拒绝上报")
    p.add_argument("id", help="上报 ID")
    p.add_argument("--reason", help="拒绝原因")
    p.set_defaults(func=cmd_escalation_reject)
